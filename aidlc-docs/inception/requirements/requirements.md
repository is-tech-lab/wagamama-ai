...
> **Note**: 旧 B2「代理コミュニケーション（チャットアプリ自動返信）」は MVP 対象外。チャットアプリ個人 API の制約により実装現実性が低く、F3 と機能重複するため §4.3 Out of Scope へ移動、将来展望は §6 を参照。
...
- 🍽 **ママが昨晩のうちに食べログで店を予約しといた**、ランチも**ママが Uber Eats で頼んどいた**
...
| B2. 代理コミュニケーション（チャットアプリ自動返信、旧 F4） | LINE/iMessage/Slack DM 等の主要チャットアプリは個人アカウント向け公式 API が存在しないか TOS 制約が強く、ハッカソン期間内の実装現実性が低い。F3（Nova Act）が「実行代行」「判断機会の剥奪」を独占的に体現するため、機能重複の解消も兼ねて MVP 外。**将来展望は §6 参照** |
...
| 機能 | 概要 |
|------|------|
| 代理コミュニケーション（旧 B2 / F4） | 友人への返信・連絡・催促をママが代筆・代行（チャットアプリ公式 API・OAuth 同意フロー整備後、または自前チャット圏内で展開） |
